...
const ArtistRow = ({artist, selectedArtistID, setSelectedArtistID}: ArtistRowProps) => {
  ...
  */
  const row = (
    <tr onClick={()=>setSelectedArtistID(artist.id)}>
        <td>{artist.name}</td>
        <td>{artist.images.length ? <img src={artist.images[0].url}></img> : "No image found"}</td>
        <td><input type="radio" name="selected_artist" value={artist.id} required readOnly checked={artist.id === selectedArtistID}
